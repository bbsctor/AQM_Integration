...
public class LoginModel extends BaseModel {
    ...
    public boolean login(@NonNull String username, @NonNull String pwd, @NonNull final InfoHint
        ...
        httpService.login(username, pwd)
                .compose(new CommonTransformer<TokenWrapper>())
                .subscribe(new CommonSubscriber<TokenWrapper>(ProApplication.getmContext()) {

                    @Override
                    public void onNext(TokenWrapper token) {
                        isLogin = true;
                        infoHint.successInfo(token.getCredentials());
                    }
